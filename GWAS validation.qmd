...
GWAS_magma_pvals_path <- paste0(
  analysis_folder_path,
  "FUMA results/GWAS_intersection/Useful_files/GWAS_gene_pvals.csv"
)

DisGeNET_magma_pvals_path <- paste0(
  analysis_folder_path,
  "FUMA results/DisGeNET_intersection/Useful_files/DisGeNET_gene_pvals.csv"
)

Clinvar_magma_pvals_path <- paste0(
  analysis_folder_path,
  "FUMA results/Clinvar_intersection/Useful_files/Clinvar_gene_pvals.csv"
)

Union_magma_pvals_path <- paste0(
  analysis_folder_path,
  "FUMA results/Union_intersection/Useful_files/Union_gene_pvals.csv"
)

Common_genes_magma_pvals_path <- paste0(
  analysis_folder_path,
  "FUMA results/Common_genes_intersection/Useful_files/common_predicted_gene_pvals.csv"
)
```
...
magma_gene_pval_file_paths <- c(
  ...
  Common_genes_magma_pvals = Common_genes_magma_pvals_path
)
```

```{r Read in MAGMA pval files}
magma_gene_pval_files <- magma_gene_pval_file_paths %>% 
  map(~ read_csv(.x))
...
```{r Create list of predicted schziophrenia genes with the NAs removed}
...
```{r Find the common predicted genes across the 4 lists}
# Find the intersection of the four gene lists
common_predicted_genes <- Reduce(
  intersect,
  predicted_schizophrenia_gene_list
  ) %>% 
  as_tibble()
...
```{r Append common predicted genes to predicted gene list}
predicted_schizophrenia_gene_list[["Common_predicted_genes"]] <- common_predicted_genes$value
...
magma_gene_pval_files_mapped <- magma_gene_pval_files %>% 
...
training_genes[["common_genes"]] <- "PLACEHOLDER"
...
original_GWAS_pvals <- read_csv(
  file = paste0(
    analysis_folder_path,
    "FUMA results/FUMA_original_GWAS/Useful_files/original_GWAS_pvals.csv"
  )
)
...
write.xlsx(
  ...
  file = paste0(
    analysis_folder_path,
    "Validation analyses/GWAS_validation_results/MAGMA_gene_based_summary.xlsx"
  )
)
```
...
In this context, gene set 1 will be all the genes in the SNP2Gene data for all the schizophrenia summary statistics that were not in the SNP2Gene data for the intersection data from another list. Gene 2 will be all the genes that were obtained from SNP2Gene after submitting an intersection file.
...
Gene_set_2_list <- magma_gene_pval_files %>% 
  map(function(x){
